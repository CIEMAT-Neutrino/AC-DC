...
def load_data(numbers:list,labels:list,ovs:list,chs:list,path:str=f'{root}/data/',debug:bool=False):
    return load_df('',numbers,labels,ovs,chs,path,debug)

def load_fit(numbers:list,labels:list,ovs:list,chs:list,path:str=f'{root}/data/',debug:bool=False):
    return load_df('_fit',numbers,labels,ovs,chs,path,debug)

def load_analysis(numbers:list,labels:list,ovs:list,chs:list,path:str=f'{root}/data/',debug:bool=False):
    return load_df('_analysis',numbers,labels,ovs,chs,path,debug)

def load_df(stage:str,numbers:list,labels:list,ovs:list,chs:list,path:str=f'{root}/data/',debug:bool=False):
    import pandas as pd
    from itertools import product
    df = pd.DataFrame()
...
def get_duration(nunbers:list,labels:list,chs:list,ovs:list,ow:bool=True,debug:bool=False):
    for n, label, ch, ov in product(nunbers, labels, chs, ovs):
        try:
            times = yaml.load(open(f'{root}/data/analysis/times.yml'),Loader=yaml.FullLoader)
        ...
        with open(f'{root}/data/analysis/times.yml', 'w') as file:
            documents = yaml.dump(times, file)
    
    os.chmod(f'{root}/data/analysis/times.yml', stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)
    return times